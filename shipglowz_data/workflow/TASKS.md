...
- [ ] gocharbon: design-system authority hardening
  ...
  - area: design
  - scope: website
  - action: harden design rules so UI changes must use centralized tokens from `site/src/styles/global.css` and `shipglowz_data/technical/site/design-system-authority.md`
  - linked_specs:
...
The following content was preserved from `/home/claude/shipglowz_data/projects/gocharbon/TASKS.md` during central repository retirement. Treat it as historical backlog/context unless an item is promoted into the active section above.
